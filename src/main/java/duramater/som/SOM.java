package duramater.som;//SOM.java
/**
...
public class SOM extends Grid
{
	...
	// The disance from each observations point to the final node
	...
	/**
	 * Train the SOM to the observations.
	 * This method runs the training
	 * algorithm to fit the self-organizing
	 * maps to the training observations
	 * 
	 * */
	public void train()
	{
		this.init();
		// Number of rows in the training observations
		int dataRows = this.gridData.length;
		// Number of columns in the training observations
		...
		// Now calculate the weights/observations to map distance
		count = 0;
		double [] mapDist = new double[dataRows * weightsRows];
		// Loop over all observations points
	...
	private void init()
	{
		...
		// Sample from the observations to determine
