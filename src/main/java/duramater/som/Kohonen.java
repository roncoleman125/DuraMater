 ...
 * numeric observations in a csv format.
 * 
 * 
 * This file contains
 * the main method and constructs the necessary
 * windows for user input as well as reading in
 * the observations and validating it. It then passes
 * the necesary observations onto a SOM object where
...
// See https://github.com/dashaub/kohonen4j
public class Kohonen extends JFrame
{
	...
	/**
	 * Read in the input csv observations
	 * This function reads the input observations
	 * from a csv for constructing the
	 * self-organizing map. The function also
	 * checks the observations to make sure it is valid
	...
	public void readFile(File input)
	{
		...
		while(inFile.hasNext())
		{
			...
			try
			{
				...
				// Ignore observations in rows with more entries than in the header
			...
			// Ensure the observations are parsed to numeric
			catch(NumberFormatException nfe)
			{
				JOptionPane.showMessageDialog(null, "The file should contain only numeric observations.");
				return;
			}
		...
		{
			JOptionPane.showMessageDialog(null, "There must be at least as many observations rows as columns in the file.");
			return;
		}
		
		// The observations has passed validity checks, so convert to an array
